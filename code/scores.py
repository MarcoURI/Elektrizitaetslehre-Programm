...
class Scores:
    ...
    def __init__(self, spanscore=0, widerscore=0, stromscore=0, gemischscore=0
                 ,spanwrong=0, widerwrong=0, stromwrong=0, gemischwrong=0):
        self.spanscore = spanscore
        self.widerscore = widerscore
        self.stromscore = stromscore
        self.gemischscore = gemischscore
        
        self.spanwrong = spanwrong
        self.widerwrong = widerwrong
        self.stromwrong = stromwrong
        self.gemischwrong = gemischwrong
                
        self.timestamps = {
            'span richtig': None,
            'wider richtig': None,
            'strom richtig': None,
            'gemisch richtig': None,
    ...
    def add_gemisch(self, points=1):
        self.gemischscore += points
        self.timestamps['gemisch richtig'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.save_scores()
        return self.gemischscore
 
    def add_spannwrong(self, points=1):
    ...
    def add_gemischwrong(self, points=1):
        self.gemischwrong += points
        self.timestamps['gemisch falsch'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.save_scores()
        return self.gemischwrong
        ...
        try:
            with open("scores.txt", "w") as file:
                scores_data = [
                    ...
                    f"Gemisch richtig: {self.gemischscore} | Last Updated: {self.timestamps['gemisch richtig']}",
                    ...
                    f"Gemisch falsch: {self.gemischwrong} | Last Updated: {self.timestamps['gemisch falsch']}"
                ]
                file.write("\n".join(scores_data))
        ...
        try:
            with open("scores.txt", "r") as file:
                ...
                for line in lines:
                    ...
                    elif "Gemisch richtig:" in line:
                        parts = line.split("|")
                        self.gemischscore = int(parts[0].split(":")[1].strip())
                        self.timestamps['gemisch richtig'] = parts[1].split(":")[1].strip()
                    ...
                    elif "Gemisch falsch:" in line:
                        parts = line.split("|")
                        self.gemischwrong = int(parts[0].split(":")[1].strip())
                        self.timestamps['gemisch falsch'] = parts[1].split(":")[1].strip()
